...
class Gan():
        ...
        def pre_process_data(self, dataframe = None):
            ...
            ymal_test = ymal_test.values
            return {'xmal_test': xmal_test, 'ymal_test': ymal_test}

        def test(self, choose = 'blackbox'):
            if choose == 'blackbox':
                data_processed = self.pre_process_data(self.dataset3)
                xmal_test = data_processed["xmal_test"]
                ymal_test = data_processed["ymal_test"]
                input_test_score = self.blackbox_detector.score(xmal_test, ymal_test)
            else:
                data_processed = self.pre_process_data(self.dataset2)
                xmal_test = data_processed["xmal_test"]
                ymal_test = data_processed["ymal_test"]
                input_test_score = self.ganD.score(xmal_test, ymal_test)
            return input_test_score

        def GAN_generator(self):
            pre_process_data(dataframe = '')
            #self.blackbox_detector.fit(xmal_test, ymal_test.values)
            #input_test_score1 = self.blackbox_detector.score(xmal_test, ymal_test.values)
            #modifying features
            ...
            print('Actual_TPR of test dataset: {0}'.format(input_test_score))
            #print('Actual_TPR after Training: {0}'.format(input_test_score1))
        ...
        def train(self, choose = 'blackbox'):
            if choose == 'ganD':
                ymal_test = self.test(xmal_test, ymal_test, choose = 'blackbox')
                xmal_test = self.pre_process_data(dataframe = self.dataset2)['xmal_test']
                self.ganD.fit(xmal_test, ymal_test)
            else:
                data_processed = self.pre_process_data(dataframe = self.dataset1)
                xmal_test = data_processed["xmal_test"]
                ymal_test = data_processed["ymal_test"]
                self.blackbox_detector.fit(xmal_test, ymal_test)
            train_score = self.blackbox_detector.score(xmal_test, ymal_test)
            print("+++++++++++++++++Train_score++++++++++++++++++++++")
...
if __name__ == "__main__":
        ...
        ganObject.train(choose = 'blackbox')
        ganObject.test(choose = 'blackbox')
